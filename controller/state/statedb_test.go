...
func TestLoadTask(t *testing.T) {
	...
	withState(ctx, t, func(state *stateDB) {
		...
		rt := tasks.NewRetrievalTask("t01000", "bafk2bzacedli6qxp43sf54feczjd26jgeyfxv4ucwylujd3xo5s6cohcqbg36", false, "")
		tsk := tasks.NewTask(rt, nil)
...
func TestAssignTask(t *testing.T) {
	...
	withState(ctx, t, func(state *stateDB) {
		...
		for i := 0; i < taskCount; i++ {
			...
			req := tasks.NewPopTask(worker, tasks.InProgress)
		...
		task, err = state.AssignTask(ctx, tasks.NewPopTask("it's me", tasks.InProgress))
		require.NoError(t, err)
...
func TestAssignTaskWithTag(t *testing.T) {
	...
	withState(ctx, t, func(state *stateDB) {
		...
		tasktag := "testtag"
		rt := tasks.NewRetrievalTask("t01000", "bafk2bzacedli6qxp43sf54feczjd26jgeyfxv4ucwylujd3xo5s6cohcqbg36", false, tasktag)
		task := tasks.NewTask(rt, nil)
		err := state.saveTask(ctx, task, tasktag, "")
		require.NoError(t, err)

		tasktag = "sometag"
		rt = tasks.NewRetrievalTask("f0127896", "bafk2bzacedli6qxp43sf54feczjd26jgeyfxv4ucwylujd3xo5s6cohcqbg36", false, tasktag)
		task = tasks.NewTask(rt, nil)
		err = state.saveTask(ctx, task, tasktag, "")
		require.NoError(t, err)
		...
		worker := "tester"
		req := tasks.NewPopTask(worker, tasks.InProgress, "foo", "bar")
		require.True(t, req.Tags != nil, "Tags does not exist in request")
		task, err = state.AssignTask(ctx, req)
		require.NoError(t, err)
		require.Nil(t, task, "Shoud not get task with tags that do not match search")

		// Should get tagged task with matching tags
		req = tasks.NewPopTask(worker, tasks.InProgress, "foo", "bar", "testtag")
		task, err = state.AssignTask(ctx, req)
	...
	withState(ctx, t, func(state *stateDB) {
		...
		for i := 0; i < taskCount; i++ {
			go func(n int) {
				worker := fmt.Sprintf("worker-%d", n)
				<-release
				req := tasks.NewPopTask(worker, tasks.InProgress, "testtag")
...
func TestUpdateTasks(t *testing.T) {
	...
	withState(ctx, t, func(state *stateDB) {
		...
		exStageDetail := tasks.NewStageDetails("Doing Stuff", "A good long while")
		...
		_, err = state.Update(ctx, inProgressTasks[2].GetUUID(),
			tasks.NewUpdateTaskWithStage(*(inProgressTasks[2].WorkedBy), tasks.InProgress, "", "Stuff", exStageDetail, 1))
		require.NoError(t, err)
		...
		testCases := []struct {
		...
		}{
			{
				...
				updateTaskRequest: tasks.NewUpdateTask("tester", tasks.InProgress, 1),
				expectedError:     ErrNotAssigned,
			},
			{
				name:              "attempting to work on task with another worker",
				uuid:              inProgressTasks[0].GetUUID(),
				updateTaskRequest: tasks.NewUpdateTask("tester 2", tasks.Successful, 1),
			...
			},
			{
				...
				updateTaskRequest:    tasks.NewUpdateTaskWithStage(*(inProgressTasks[1].WorkedBy), tasks.InProgress, "", "Stuff", exStageDetail, 1),
				expectedStage:        "Stuff",
			...
			},
			{
				...
				updateTaskRequest:    tasks.NewUpdateTaskWithStage(*(inProgressTasks[2].WorkedBy), tasks.InProgress, "", "Stuff", workedStageDetail, 1),
				expectedStage:        "Stuff",
			...
			},
			{
				...
				uuid:                 inProgressTasks[2].GetUUID(),
				updateTaskRequest:    tasks.NewUpdateTaskWithStage(*(inProgressTasks[2].WorkedBy), tasks.Failed, "Something went wrong", "Stuff", workedStageDetail, 1),
			...
			t.Run(tc.name, func(t *testing.T) {
				...
				if tc.expectedError != nil {
					...
					if tc.expectedErrorMessage == "" {
						require.False(t, task.ErrorMessage != nil)
					} else {
						require.Equal(t, tc.expectedErrorMessage, *(task.ErrorMessage))
					}
					require.Equal(t, tc.expectedStage, task.Stage)
					if tc.expectedStageDetails == nil {
						require.Equal(t, task.CurrentStageDetails != nil, false)
					} else {
						require.Equal(t, tc.expectedStageDetails, task.CurrentStageDetails)
					}
...
`
		...
		stage1 := tasks.NewStageDetails("Doing Stuff", "A good long while").WithLog("stuff happened")
		stage2 := tasks.NewStageDetails("Doing More Stuff", "A good long while").WithLog("more stuff happened")

		_, err = state.Update(ctx, inProgressTask2.GetUUID(),
			tasks.NewUpdateTaskWithStage(*(inProgressTask2.WorkedBy), tasks.InProgress, "", "Stage1", stage1, 1))
		require.NoError(t, err)
		_, err = state.Update(ctx, inProgressTask2.GetUUID(),
			tasks.NewUpdateTaskWithStage(*(inProgressTask2.WorkedBy), tasks.InProgress, "", "Stage2", stage2, 1))
		require.NoError(t, err)

		// pop a task and set it failed
		req = tasks.NewPopTask(worker, tasks.Failed)
		failedTask, err := state.AssignTask(ctx, req)
		require.NoError(t, err)
		failedTask, err = state.Update(ctx, failedTask.GetUUID(), tasks.NewUpdateTask(*(inProgressTask2.WorkedBy), tasks.Failed, 1))
		require.NoError(t, err)
		...
		require.NoError(t, err)
		successfulTask, err = state.Update(ctx, successfulTask.GetUUID(), tasks.NewUpdateTask(*(inProgressTask2.WorkedBy), tasks.Successful, 1))
		require.NoError(t, err)

		// pop two tasks to the other worker and leave them in progress
		req = tasks.NewPopTask(otherWorker, tasks.InProgress)
		...
		require.Equal(t, "", *(inProgressTask1.WorkedBy))
		...
		require.Equal(t, "", *(inProgressTask2.WorkedBy))
		require.Equal(t, "", inProgressTask2.Stage)
		require.False(t, inProgressTask2.CurrentStageDetails != nil)
		require.False(t, inProgressTask2.PastStageDetails != nil)
		...
		require.Equal(t, "", *(unassignedTask.WorkedBy))
...
func TestComplete(t *testing.T) {
	...
	withState(ctx, t, func(state *stateDB) {
		...
		task, err := state.AssignTask(ctx, tasks.NewPopTask("dealbot1", tasks.InProgress))
		require.NoError(t, err)
		require.Equal(t, tasks.InProgress, task.Status)

		// succeed task.
		task, err = state.Update(ctx, task.GetUUID(), tasks.NewUpdateTask("dealbot1", tasks.Successful, 1))
		require.NoError(t, err)
		...
		require.NoError(t, err)
		require.True(t, nextHead.Previous == nil)
		require.Equal(t, int64(1), int64(len(nextHead.Records)))
		record := nextHead.Records[0]
		store := state.Store(ctx)
		blk, err := store.Get(context.TODO(), record.Record.(cidlink.Link).Cid.String())
		require.NoError(t, err)
		tskBuilder := tasks.FinishedTaskPrototype.NewBuilder()
		require.NoError(t, dagjson.Decode(tskBuilder, bytes.NewReader(blk)))
		tsk, err := tasks.UnwrapFinishedTask(tskBuilder.Build())
		require.Equal(t, task.RetrievalTask.PayloadCID, *(tsk.PayloadCID))
	})
}
...
func populateTestTasks(ctx context.Context, stream io.Reader, state State) error {
	...
	for _, task := range byTask {
		...
		if err = dagjson.Decode(rtp, bytes.NewBuffer(task)); err != nil {
			...
			stNode, err := tasks.UnwrapStorageTask(stp.Build())
			if err != nil {
				return err
			}
			if _, err = state.NewStorageTask(ctx, stNode); err != nil {
				return err
			}
		} else {
			rtNode, err := tasks.UnwrapRetrievalTask(rtp.Build())
			if err != nil {
				return err
			}
			if _, err = state.NewRetrievalTask(ctx, rtNode); err != nil {
				return err
			}
		}
	}
