...
const (
	...
	createStatusLedgerSQL = `
		...
		CREATE UNIQUE INDEX idx_task_status_ledger_by_stage ON task_status_ledger (uuid, status, stage_name);
	`
	...
	setTaskStatusSQL = `
		INSERT INTO task_status_ledger (uuid, status, stage_name, ts) VALUES($1, $2, '', $3)
	`

	upsertTaskStatusSQL = `
		INSERT INTO task_status_ledger (uuid, status, stage_name, ts) VALUES($1, $2, $3, $4)
	`
	...
	currentTaskStatusSQL = `
		SELECT status, stage_name FROM task_status_ledger WHERE uuid = $1 ORDER BY ts DESC limit 1
	`
	...
	taskHistorySQL = `
		SELECT status, stage_name, ts FROM task_status_ledger WHERE uuid = $1 ORDER BY ts
	`
)
